...
# Per-axis evaluation checklists. Each item is testcase-independent.
# Used by prompt.py to build a static block injected into the user message.
AXIS_CHECKLISTS: dict[str, tuple[str, ...]] = {
    "correctness": (
...
AXIS_TOTAL_MAX: int = AXIS_MAX * len(AXES)   # 40 with current 4 axes


# ----- Score formulas (single source of truth, used by prompt + post-processing) -----
...
def overall_score(scores: dict[str, int]) -> int:
    """0~100 정수. 4축 단순 평균 → 100 환산."""
    total = sum(int(scores.get(a, 0)) for a in AXES)
    return round(total / AXIS_TOTAL_MAX * 100)


def partial_score(total_score: int, scores: dict[str, int]) -> int:
    """testcase 미달분 보상용 부분점수. correctness + problem_understanding 만의 함수."""
    if total_score <= 0:
        return 0
...
class Rubric:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "checklists": {a: list(items) for a, items in AXIS_CHECKLISTS.items()},
            "overall_formula": OVERALL_FORMULA_TEXT,
            "partial_formula": PARTIAL_FORMULA_TEXT,
        }
