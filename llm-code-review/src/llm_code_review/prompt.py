"""Hierarchical prompt builders for the qualitative evaluation pass.

Structure (Phase 2):
    system_message  := SYSTEM_PRINCIPLES (static)
    user_message    := build_problem_block(task)
                     + build_code_block(task)
                     + build_axis_checklists()        (static)
...
from llm_code_review.rubric import (
    AXES,
    AXIS_CHECKLISTS,
    AXIS_DESCRIPTIONS,
    AXIS_RANGE,
...
def build_axis_checklists() -> str:
    """Static block: per-axis evaluation checklist."""
...
def build_problem_block(task: EvalTask) -> str:
    p = task.problem
    return f"""[문제]
...
def build_code_block(task: EvalTask) -> str:
    s = task.submission
...
def build_evaluation_instructions(task: EvalTask) -> str:
    return f"""[평가 지시]
...
def build_response_schema(task: EvalTask) -> str:
    schema_doc = {
        "scores": {a: f"int in [{AXIS_RANGE[0]},{AXIS_RANGE[1]}]" for a in AXES},
        "comments": {
            a: {
                "assessment": "string (한국어 1~2문장, 비공백 필수)",
                "suggestion": "string (한국어 1~2문장, 비공백 필수)",
            }
            for a in AXES
        },
    ...
    # Concrete one-shot example so the model gets nesting right.
    example = {
        "scores": {a: 7 for a in AXES},
        "comments": {
            a: {
                "assessment": "예시: 현재 상태에 대한 한국어 평가 한두 문장.",
                "suggestion": "예시: 구체적 개선 방향 한두 문장.",
            }
            for a in AXES
        },
        "overall": 70,
