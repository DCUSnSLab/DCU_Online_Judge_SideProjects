"""AI 사용 가능성 평가 — 정성평가와 분리된 별도 LLM 호출.

설계 원칙:
- 점수 계산에 일체 영향 X. 정성평가 결과 JSON 의 형제 필드로 저장.
- 결과는 채점자 참고용. `disclaimer` 필드를 항상 포함.
- 한쪽 관점으로 추론하지 않도록 `counter_signals` 를 필수.
- 평가 관점들은 "참고 신호 — 단독 결정적이지 않음" 라벨과 함께 제시.
"""
...
from llm_code_review.llm import LLMResponseError, extract_json_object
from llm_code_review.models import AIUsageAssessment, AIUsageSignal, EvalTask
from llm_code_review.prompt import _LANG_MD, html_to_text
...
def _build_user_prompt(task: EvalTask) -> str:
    p = task.problem
    s = task.submission
    ...
    schema = {
        ...
        "signals": [
            {
                "category": " | ".join(_SIGNAL_CATEGORIES),
                "observation": "string (한국어 1문장)",
...
def _check_str(v: Any, where: str, *, allow_empty: bool = False) -> str:
...
def parse_response(text: str) -> dict[str, Any]:
    ...
    counter_signals = [
        _check_str(c, f"counter_signals[{i}]") for i, c in enumerate(counter_signals_raw)
    ]
    ...
    return {
        ...
        "disclaimer": DISCLAIMER_TEXT,  # always normalize to canonical text
    }


def make_assessment(parsed: dict[str, Any], *, raw: str, latency_ms: int, model: str) -> AIUsageAssessment:
...
def make_failed_assessment(error: str, *, model: str) -> AIUsageAssessment:
    return AIUsageAssessment(
        likelihood_score=0,
        confidence="low",
        signals=[],
